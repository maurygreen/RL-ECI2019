...
class Agent():
    """
    Interacts with and learns from the environment. Modification of the class
    Agent provided in the example code for DQN_lunarlander.
    """


    def __init__(self, state_size, action_size, seed, network, optim_fn='adam'):
        ...
        ======
            ...
            network (class): class of the network implemented with PyTorch.
                             Must be a subclass of torch.nn.Module
            optim_fn (string): name of the optimization algorithm. One of
                               {'adam','rmsprop'}. Default: 'adam'.
        """
        self.state_size = state_size
        self.action_size = action_size
        self.seed = random.seed(seed)

        # Q-Network, using the class network
        ...
        # Set the optimizer to the selected one
        if optim_fn == 'adam':
            self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR)
        elif optim_fn == 'rmsprop':
    ...
    def plot_net(self):
        samp = torch.zeros(self.state_size)
        out = self.qnetwork_local(samp)
        make_dot(out, params=dict(self.qnetwork_local.named_parameters()))
